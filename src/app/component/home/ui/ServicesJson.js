    ...
    <div id="productWrapper" className="grid gap-5 md:grid-cols-3 w-full">
      {filteredProducts.map((product) => (
        <div
          key={product.id}
          id={product.id}
          className={`bg-base-100 rounded-none rounded-bl-3xl ${product.isBest === true ? "border border-amber-300 shadow-xl shadow-amber-300/50" : ""}`}
          data-aos="fade-up"
        >
                ...
                {product.benefits
                  ...
                  .map((b) => (
                    <li key={b.id} className="flex items-center gap-1">
                      {b.isActive ? <FaCheck className="text-green-500" /> : <FaXmark className="text-red-500" />}
