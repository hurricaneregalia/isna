      ...
      {listItem.map((item, index) => (
        <div className="col-span-1 flex justify-stretch" key={index} data-aos="fade-up">
          <div className={`bg-base-100 lg:p-20 lg:pb-0 sm:p-10 sm:pb-0 p-8 pb-0 rounded-bl-3xl overflow-hidden w-full ${borderFx}`}>
            ...
            <div>
              <p className="font-bold mb-3 my-2 text-xl h1" data-aos="fade-up">{String(index + 1).padStart(2, "0")}.</p>
              <p className="font-bold mb-3 my-2 text-xl h3" data-aos="fade-up">{item.title}</p>
              <p className="mb-20" data-aos="fade-up">{item.desc ? item.desc : item.description}</p>
            </div>
            <div className="" >
              <ImageComponent
                imageUrl={item.img}
