...
import ThemeSwitch from "@/app/component/global/themeSwitch";
...
export default function ExalviaNavbar({ data, bgCustom = "bg-transparent" }) {
  ...
  return (
    <div className="relative w-full">
      <nav
        className={`fixed left-0 right-0 sm:mx-4 mx-2 z-10 transition-all duration-300 rounded-4xl  sm:mt-4 mt-2 rounded-tr-none border-b ${
          scrolled ? "bg-secondary py-5 px-0 shadow-lg border-base-100/20" : bgCustom + " py-8 px-2 border-transparent"
        }`}
      >
        <div className={`lg:w-10/12 w-full mx-auto px-6 sm:px-16  transition-all duration-300 lg:px-24 ${scrolled ? "mt-0" : "mt-0"} `}>
          <div className="flex items-center justify-between">
            {/* 1. Left: Brand */}
            <div className="flex-1 flex items-center">
              <div className="flex items-center cursor-pointer group" onClick={() => scrollToSection("hero")}>
                <div className="relative w-55 h-8 text-start flex justify-start">
                  <Image src={data?.logoFullLight} alt={data?.brandName} fill className="object-contain object-left sm:block hidden" />
                  <Image src={data?.logoGramLight} alt={data?.brandName} fill className="object-contain object-left sm:hidden block mx-0 text-start" />
                </div>
          ...
          <div className="flex flex-col h-full p-10 space-y-12 items-center justify-center text-center relative">
            ...
            >
              <div className="flex items-center gap-2 cursor-pointer group mb-20">
                <div className="relative w-68 h-10 ">
                  <Image src={data?.logoFullDark} alt={data?.brandName} fill className="object-contain" />
                </div>
              </div>
            </div>
            ...
            <ExalviaLinkButton text="Konsultasi" href="#contact" className="mt-4 btn-warning btn-lg" onClick={() => setIsOpen(false)} />
          </div>
        </div>
      </nav>
    </div>
