...
export default function AlternativePackages({ packages, isOpen, onToggle, formatCurrency, onDetailClick }) {
  return (
    <div className="join join-vertical bg-base-200 rounded-4xl rounded-tr-none overflow-hidden p-0 m-0">
        ...
        <div className="collapse-content text-sm p-0">
          <div className="grid grid-cols-1 gap-2 p-4 pt-0">
            {packages.map((pkg, index) => (
              <div key={index} className="sm:p-8 p-4  bg-base-100  rounded-2xl rounded-tr-none flex sm:flex-row flex-col items-start w-full">
