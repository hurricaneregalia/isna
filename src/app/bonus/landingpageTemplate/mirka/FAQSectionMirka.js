...
export default function FAQSectionMirka({ data, secId }) {
  ...
  return (
    <section id={secId}>
      <div className="bg-base-100 lg:px-20 py-32">
          ...
          <div className="lg:w-8/12 sm:w-1/2 w-full">
            <div className="join join-vertical w-full ">
              {data.faqs.map((faq, index) => (
                <div
                  key={index}
                  className={`collapse join-item border-base-300 border-t rounded-none text-base-content transition-all duration-300 ${activeIndex === index ? "collapse-open" : "collapse-close"}`}
                >
