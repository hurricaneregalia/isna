        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 auto-rows-[200px] sm:gap-4 space-y-4 sm:space-y-0">
          {photos.map((photo, idx) => (
            <div key={idx} className={`relative overflow-hidden rounded-xl group ${photo.className}`}>
              ...
              />
              <div
                className="absolute bottom-0 left-0 w-full bg-linear-to-t from-neutral-900/70 to-transparent text-white text-sm sm:text-base px-4 py-5 transition-opacity duration-300 group-hover:opacity-0
  "
              >
                {photo.caption}
              </div>
