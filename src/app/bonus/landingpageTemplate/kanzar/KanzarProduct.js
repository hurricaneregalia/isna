...
const KanzarProduct = ({ onOrder, secId = "product", data }) => {
  const { title, subtitle, products, border } = data.product;

  return (
    <>
      <section id={secId} className="bg-base-200 transition-colors duration-300 relative">
        <div className="w-full h-[50px] mb-24" style={{ backgroundImage: `url(${border})`, transform: "rotate(180deg)", backgroundPosition: "center" }} />
        ...
        <div className="w-full h-[50px] mt-24" style={{ backgroundImage: `url(${border})`, backgroundPosition: "center" }} />
      </section>
    </>
  );
};
