...
const iconMap = {
  trust: HiShieldCheck,
  conversion: HiTrendingUp,
};

export default function ExalviaFact({ data }) {
  if (!data) return null;

  return (
    <section id="fact" className="py-20 md:py-32 relative overflow-hidden bg-base-100">
      <div className="lg:w-10/12 sm:w-11/12 w-full mx-auto px-6 md:px-16 lg:px-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {data.items?.map((item, index) => (
                <ExalviaCard key={index} className="flex items-center gap-4 p-4 md:p-5">
                  <ExalviaIconBox icon={iconMap[item.icon]} className="shrink-0" />
          ...
          <div className="order-1 lg:order-2 relative">
            ...
            <div className="absolute -bottom-6 -left-6 w-32 h-32 bg-primary/10 rounded-full blur-none hidden md:block"></div>
