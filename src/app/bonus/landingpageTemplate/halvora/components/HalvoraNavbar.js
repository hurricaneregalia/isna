...
export default function HalvoraNavbar({ data, children }) {
  ...
  return (
    <div className="relative w-full">
      <nav className={`fixed w-full z-50 transition-all duration-300 ease-in-out ${isScrolled ? "bg-base-200/95 shadow-sm py-3" : "bg-transparent py-6"}`}>
        ...
        <div className={`w-full px-5 transition-all duration-300 `}>
          <div className="container mx-auto lg:w-10/12 w-full py-3 flex justify-between items-center">
            {/* Logo */}
            <div className={`text-2xl font-bold  font-serif tracking-in-expand ${isScrolled ? "text-base-content" : "text-white"}`}>{brand.name}</div>
              ...
              {navItems.map((item, index) => (
                <a
                  ...
                  className={`transition-colors font-medium text-sm tracking-wide ${
                    activeSection === item.href ? "text-primary font-bold" : isScrolled ? "text-base-content hover:text-primary" : "text-white hover:text-primary"
                  }`}
                >
                  {item.label}
                </a>
        ...
        {/* Mobile Menu */}
        <div
          className={`lg:hidden absolute top-full left-0 w-full h-screen bg-base-200/95 flex flex-col p-6 border-t border-primary/20 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
          ...
          <div className="flex flex-col gap-1 w-full justify-start pt-4">
            {navItems.map((item, index) => (
              <a
                key={index}
                href={item.href}
                className={`py-4 text-center text-xl border-b border-primary/10 transition-all duration-300 ${
                  activeSection === item.href ? "text-primary font-bold scale-105" : "text-base-content hover:text-primary hover:scale-105"
                }`}
