...
export default function HalvoraBestFor({ data, secId }) {
  const { title, subtitle, items } = data.bestFor;

  return (
    <section id={secId} className="py-24 bg-[#FFF5EA]">
      <div className="container mx-auto px-6 lg:w-8/12 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {items.map((item) => (
            <div 
              key={item.id} 
              className="group relative overflow-hidden rounded-2xl bg-white shadow-sm hover:shadow-xl transition-all duration-500 border border-[#D48F8F]/20"
            >
              ...
              <div className="absolute bottom-0 left-0 w-full p-6 translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                ...
                <p className="text-[#FFF5EA] text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 leading-relaxed">
                  {item.description}
                </p>
              </div>
            </div>
