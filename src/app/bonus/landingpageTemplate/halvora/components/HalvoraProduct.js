...
export default function HalvoraProduct({ data, secId }) {
  ...
  return (
    <section id={secId} className="py-24 bg-white">
      <div className="container mx-auto px-6 lg:w-8/12 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {items.map((product) => (
            <div key={product.id} className="card bg-white rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-500 group border border-[#F0E0D0] ring-1 ring-[#6B4423]/5 hover:ring-[#D48F8F]/30">
              <figure className="relative aspect-[4/5] overflow-hidden">
                ...
                />
                {product.tag && (
                    <div className="absolute top-4 right-4 bg-[#6B4423] text-white/95 p-1 px-2 rounded-full text-sm leading-relaxed">
                        {product.tag}
                    </div>
                )}
              ...
              <div className="p-8 space-y-4">
                ...
                <p className="text-sm text-[#A07050] line-clamp-2 min-h-[2.5rem] leading-relaxed">
                    {product.description}
                </p>
